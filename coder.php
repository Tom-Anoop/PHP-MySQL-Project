...
<?php  
  ...
  $name=$_POST['name'];
   ...
   if($row[0]==0)
   {
         $qry="insert into tblcoder (cEmail,cName,cAddress,cContact,cImg) values('$email','$name','$address','$contact','$file')";
         $res=mysql_query($qry);
         if($res)
         {
             $qry="insert into tbllogin (username,password,utype,status) values('$email','$pwd','coder','0')";
             $res=mysql_query($qry);
             if($res)
             {
                    echo '<script>alert(" Registration successfull ....");</script>';
                    echo '<script>location.href="coder.php"</script>';
             }
             else
             {
